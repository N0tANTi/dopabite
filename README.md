...
本地 API 默认使用 `http://localhost:5173` 和 `.data/dopabite.sqlite3`。生产环境变量示例见 [`deploy/api.env.example`](deploy/api.env.example)，其中 `BETTER_AUTH_SECRET` 必须使用随机密钥。
